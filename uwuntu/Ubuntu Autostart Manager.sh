...
# ============================================================
# Ubuntu / GNOME Autostart Manager + 4-Tile Diagnose-Kiosk + Hardware Check v4.4.11 + Wipe Auto v3.3
# ============================================================
...
pkill -TERM -f '/tmp/wipe-auto-' 2>/dev/null || true
pkill -TERM -f '/tmp/hardware-check\.' 2>/dev/null || true
# Snapshot gehört zum 4-Felder-Layout. Für einen sauberen Neuaufbau
# ebenfalls schließen, falls es gerade läuft.
...
class App(Gtk.Application):
    ...
    def build_overview(self):
        ...
        root.append(self.header("HARDWARE CHECK", refresh=True, version="v4.4.11"))
    ...
    def show_hotkeys(self, *_):
        ...
        shortcuts = [
            ...
            ("STRG+Q", "Network Check, Wipe Auto und Hardware Check schließen"),
        ...
        note = Gtk.Label(
            label=(
                "Hinweis: Im TASTATUR TEST sind F1, B, R, I, U, G und die "
    ...
    def handle_global_hotkey(self, action):
        ...
        # Im Tastatur-Test sind B, R, I, U, G und F1 ausschließlich normale Prüftasten.
...
# ------------------------------------------------------------
# Snapshot darf nach dem finalen Fokus nicht mehr "spät" auftauchen und
# diesen wieder stehlen. Wir warten daher auf ein echtes AT-SPI-Fenster.
# Keine feste lange Pause: sobald das Fenster 1,2s stabil ist geht es weiter.
echo "Warte kurz auf das Snapshot-Fenster ..."
...
PY
then
    echo "Snapshot-Fenster ist bereit."
else
...
EOF
    ...
    echo "  Snapshot und Hardware Check müssen zuerst stabil erschienen sein."
...
install_network_check() {
    ...
    echo "------------------------------------------------------------"
    echo
    echo "Installiere die aktuell getestete Network-Check-Version v2.5."
...
class ConnectionCard:
    ...
    def set_title_mac(self, mac=None):
        ...
        # - 00:00:00:00:00:00 -> komplette Zeile rot
        # - normale MAC -> normale Überschrift
...
class NetworkCheckApp(Gtk.Application):
    ...
    def poll_network(self):
        ...
        # Initial nur die tatsächlich aktive Standardverbindung testen.
        # Wenn LAN und WLAN gleichzeitig verbunden sind, wird also NICHT
        # direkt beides getestet.
            ...
            for dev in devices["ethernet"]:
                ...
                if (
                    ...
                    if iface == default_iface:
                        self.enqueue_test(iface, "lan", "active LAN link changed")
    ...
    def on_test_clicked(self, button):
        ...
        self.enqueue_test(default_iface, kind, "manual REFRESH")
